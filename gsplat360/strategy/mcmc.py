...
class MCMCStrategy(Strategy):
    ...
    def step_post_backward(
        ...
        if step < self.refine_stop_iter and step > self.refine_start_iter and step % self.refine_every == 0:
            ...
            if self.verbose:
                print(f"Step {step}: Added {n_new_gs} GSs. " f"Now having {len(params['means'])} GSs.")

            torch.cuda.empty_cache()

        if step >= self.refine_stop_iter and step % self.refine_every == 0 and len(params["means"]) > self.cap_max // 2:
            # remove low opacity GSs
            opacities = torch.sigmoid(params["opacities"].flatten())
            pruned_mask = opacities <= self.min_opacity
            n_pruned_gs = pruned_mask.sum().item()
            remove(params=params, optimizers=optimizers, state={}, mask=pruned_mask)
            if self.verbose:
                print(f"Step {step}: Removed {n_pruned_gs} GSs. " f"Now having {len(params['means'])} GSs.")
            torch.cuda.empty_cache()
