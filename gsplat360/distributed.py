...
def all_gather_int32(world_size: int, value: Union[int, Tensor], device: Optional[torch.device] = None) -> List[int]:
...
def all_to_all_int32(
    ...
    assert len(values) == world_size, "The length of values should be equal to world_size"

    if any(isinstance(v, int) for v in values):
        assert device is not None, "device is required for scalar input"

    # move to CUDA
    values_tensor = [(torch.tensor(v, dtype=torch.int, device=device) if isinstance(v, int) else v) for v in values]
    ...
    return [v.item() if isinstance(tensor, int) else v for v, tensor in zip(collected, values)]
...
def all_to_all_tensor_list(
    ...
    collected = [torch.empty((l, *data.shape[1:]), dtype=data.dtype, device=data.device) for l in collected_splits]
