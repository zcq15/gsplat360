...
from .utils import (
    depth_to_camera_normal,
    equir_depth_to_camera_normal,
...
def rasterization(
    ...
    if distloss:
        assert render_mode in ["ED", "RGB+ED"]
    ...
    if colors.shape[-1] > channel_chunk:
        ...
        render_colors, render_alphas, render_distort, accum_times, accum_visible, query_answers = [], [], [], [], [], []
        for i in range(n_chunks):
            colors_chunk = colors[..., i * channel_chunk : (i + 1) * channel_chunk]
            backgrounds_chunk = (
                backgrounds[..., i * channel_chunk : (i + 1) * channel_chunk] if backgrounds is not None else None
            )
            render_colors_, render_alphas_, render_distort_, accum_times_, accum_visible_, query_answers_ = (
                rasterize_to_pixels(
                    means2d,
                    conics,
                    colors_chunk,
            ...
            if i == 0:
                ...
                accum_times.append(accum_times_)
                accum_visible.append(accum_visible_)
                query_answers.append(query_answers_)
        render_colors = torch.cat(render_colors, dim=-1)
        render_alphas = render_alphas[0]  # discard the rest
        render_distort = render_distort[0]  # discard the rest
        accum_times = accum_times[0]  # discard the rest
        accum_visible = accum_visible[0]  # discard the rest
        query_answers = query_answers[0]  # discard the rest

    else:
        render_colors, render_alphas, render_distort, accum_times, accum_visible, query_answers = rasterize_to_pixels(
            means2d,
            conics,
            colors,
            opacities,
    ...
    meta.update({"accum_times": accum_times})
    meta.update({"accum_visible": accum_visible})
    meta.update({"query_answers": query_answers})

    return render_colors, render_alphas, render_distort, meta
...
def rasterization_2dgs(
    ...
    assert means.shape == (N, 3), means.shape
    assert quats.shape == (N, 4), quats.shape
    assert scales.shape == (N, 3) or scales.shape == (N, 2), scales.shape
    assert opacities.shape == (N,), opacities.shape
    assert viewmats.shape == (C, 4, 4), viewmats.shape
    assert Ks.shape == (C, 3, 3), Ks.shape
    assert render_mode in ["RGB", "D", "ED", "RGB+D", "RGB+ED"], render_mode
    ...
    if render_normals is not None:
        render_normals = F.normalize(render_normals, dim=-1, eps=1e-3)  # [..., H-2, W-2, 3]
        render_normals = torch.einsum("bhwn,bmn->bhwm", render_normals, inverse_Rt(viewmats)[:, :3, :3])
    ...
    if render_mode in ["RGB+ED", "RGB+D"]:
        ...
        render_normals_from_depth = depth_to_camera_normal(depth_for_normal, Ks)
        render_normals_from_depth = torch.einsum(
            "bhwn,bmn->bhwm", render_normals_from_depth, inverse_Rt(viewmats)[:, :3, :3]
        )
...
def equir_rasterization_2dgs(
    ...
    # Compute Ray-Splat intersection transformation.
    proj_results = equir_fully_fused_projection_2dgs(
        means,
        quats,
        scales,
        viewmats,
    ...
    if render_mode in ["RGB+ED", "RGB+D"]:
        ...
        render_normals_from_depth = equir_depth_to_camera_normal(depth_for_normal)
        render_normals_from_depth = torch.einsum(
            "bhwn,bmn->bhwm", render_normals_from_depth, inverse_Rt(viewmats)[:, :3, :3]
        )
    ...
    meta = {
        ...
        "tile_size": tile_size,
        "n_cameras": C,
        # note, the gradient of densify may be incorrect
        "gradient_2dgs": densify,  # This holds the gradient used for densification for 2dgs
    }
