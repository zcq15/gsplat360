...
def adam(
    ...
    exp_avg_sq: Tensor,
    valid: Tensor,
    step_per_param: Tensor,  # Float[Tensor, N]
    lr: float,
    b1: float,
    b2: float,
    eps: float,
) -> None:
    _make_lazy_cuda_func("adam")(param, param_grad, exp_avg, exp_avg_sq, valid, step_per_param, lr, b1, b2, eps)
...
def proj(
    ...
    width: int,
    height: int,
    camera_model: Literal["pinhole", "ortho", "fisheye", "equirectangular"] = "pinhole",
) -> Tuple[Tensor, Tensor]:
...
def rasterize_to_pixels(
    ...
    ret_visible: bool = False,
    query_values: Optional[Tensor] = None,  # [C, image_height, image_width, K]
    packed: bool = False,
    absgrad: bool = False,
    distloss: bool = False,
) -> Tuple[Tensor, Tensor]:
    ...
    if query_values is not None:
        assert tuple(query_values.shape[:3]) == (C, image_height, image_width), query_values.shape
        query_values = query_values.contiguous()
        query_dim = query_values.shape[3]
    else:
        query_dim = 0
    ...
    render_colors, render_alphas, render_distort, accum_times, accum_visible, query_answers = _RasterizeToPixels.apply(
        means2d.contiguous(),
        conics.contiguous(),
        colors.contiguous(),
    ...
    return render_colors, render_alphas, render_distort, accum_times, accum_visible, query_answers
...
class _FullyFusedProjection(torch.autograd.Function):
    ...
    def forward(
        ...
        if not calc_compensations:
            compensations = None
        ctx.save_for_backward(means, covars, quats, scales, viewmats, Ks, radii, conics, compensations, gauss_confs)
        ctx.width = width
        ctx.height = height
        ctx.eps2d = eps2d
        ctx.camera_model_type = camera_model_type
    ...
    def backward(ctx, v_radii, v_means2d, v_depths, v_conics, v_compensations):
        (means, covars, quats, scales, viewmats, Ks, radii, conics, compensations, gauss_confs) = ctx.saved_tensors
        width = ctx.width
        height = ctx.height
        eps2d = ctx.eps2d
        camera_model_type = ctx.camera_model_type
...
class _RasterizeToPixels(torch.autograd.Function):
    ...
    def forward(
        ...
        ret_visible: bool,
        query_values: Tensor,  # [C, image_height, image_width, K], Optional
        query_dim: int,
        width: int,
        height: int,
        tile_size: int,
    ...
    def backward(
        ...
        v_render_distort: Tensor,  # [C, H, W, 1]
        v_accum_times: Optional[Tensor] = None,  # [C, N]
        v_accum_visible: Optional[Tensor] = None,  # [C, N]
        v_query_answers: Optional[Tensor] = None,  # [C, N, K]
    ):
        (
            means2d,
            conics,
...
def equir_fully_fused_projection_2dgs(
    means: Tensor,  # [N, 3]
    quats: Tensor,  # [N, 4]
    scales: Tensor,
    viewmats: Tensor,
    ...
    if packed:
        ...
        return _EquirFullyFusedProjection2DGS.apply(
            means,
            quats,
            scales,
            viewmats,
...
class _EquirFullyFusedProjection2DGS(torch.autograd.Function):
    """Projects Gaussians to 2D."""
    ...
    def forward(
        ...
        radii, means2d, depths, ray_transforms, normals = _make_lazy_cuda_func("equir_fully_fused_projection_fwd_2dgs")(
            means,
            quats,
            scales,
            viewmats,
    ...
    def backward(ctx, v_radii, v_means2d, v_depths, v_ray_transforms, v_normals):
        ...
        height = ctx.height
        eps2d = ctx.eps2d
        v_means, v_quats, v_scales, v_viewmats = _make_lazy_cuda_func("equir_fully_fused_projection_bwd_2dgs")(
            means,
            quats,
            scales,
            viewmats,
...
def equir_rasterize_to_pixels_2dgs(
    means2d: Tensor,
    ray_transforms: Tensor,
    colors: Tensor,
    opacities: Tensor,
    ...
    (
        ...
        render_distort,
        render_median,
    ) = _EquirRasterizeToPixels2DGS.apply(
        means2d.contiguous(),
        ray_transforms.contiguous(),
        colors.contiguous(),
        opacities.contiguous(),
...
class _EquirRasterizeToPixels2DGS(torch.autograd.Function):
    """Rasterize gaussians 2DGS"""
    ...
    def forward(
        ...
        (
            ...
            last_ids,
            median_ids,
        ) = _make_lazy_cuda_func("equir_rasterize_to_pixels_fwd_2dgs")(
            means2d,
            ray_transforms,
            colors,
            opacities,
    ...
    def backward(
        ...
        (
            ...
            v_normals,
            v_densify,
        ) = _make_lazy_cuda_func("equir_rasterize_to_pixels_bwd_2dgs")(
            means2d,
            ray_transforms,
            colors,
            opacities,
