...
try:
    ...
    from gsplat360 import csrc as _C
except ImportError:
    ...
    if cuda_toolkit_available():
        ...
        if os.path.exists(os.path.join(build_dir, "gsplat360_cuda.so")) or os.path.exists(
            os.path.join(build_dir, "gsplat360_cuda.lib")
        ):
            ...
            with Console().status(
                f"[bold yellow]gsplat360: Setting up CUDA with MAX_JOBS={os.environ['MAX_JOBS']} (This may take a few minutes the first time)",
        ...
        Console().print("[yellow]gsplat360: No CUDA toolkit found. gsplat360 will be disabled.[/yellow]")
