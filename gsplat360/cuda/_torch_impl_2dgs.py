...
from gsplat360.cuda._torch_impl import _quat_scale_to_matrix
...
def _fully_fused_projection_2dgs(
    ...
    """PyTorch implementation of `gsplat360.cuda._wrapper.fully_fused_projection_2dgs()`
    ...
    f = torch.where(valid, test / d, torch.zeros_like(test)).unsqueeze(-1)  # (C, N, 3, 1)
    means2d = (M[..., :2] * M[..., 2:3] * f).sum(dim=-2)  # [C, N, 2]
    extents = torch.sqrt(means2d**2 - (M[..., :2] * M[..., :2] * f).sum(dim=-2))  # [C, N, 2]
...
def _rasterize_to_pixels_2dgs(
    ...
    """Pytorch implementation of `gsplat360.cuda._wrapper.rasterize_to_pixels_2dgs()`.
