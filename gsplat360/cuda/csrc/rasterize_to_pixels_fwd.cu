...
__global__ void rasterize_to_pixels_fwd_kernel(
    ...
    const S *__restrict__ query_values, // [C, image_height, image_width, K]
    ...
    int32_t *__restrict__ accum_times, // [C, N] 
    S *__restrict__ accum_visible, // [C, N]
    S *__restrict__ query_answers, // [C, N, K] 
    ...
    if (query_values != nullptr) {
        query_values += camera_id * image_height * image_width * query_dim;
    }
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            // TODO: currently, only propagated onto the Gaussian with the largest pixel response
            // // calculate query answers with cumulative visibility
            // if (query_values != nullptr && inside) {
            //     GSPLAT_PRAGMA_UNROLL
            //     for (uint32_t k = 0; k < query_dim; ++k) {
            //         gpuAtomicAdd(query_answers + g * query_dim + k,  vis * query_values[pix_id * query_dim + k]);
            //     }
            // }

            if ((query_values != nullptr || accum_visible != nullptr) && inside) {
                if (vis > max_resp_vis) {
                    max_resp_idx = g;
                    max_resp_vis = vis;
                }
            ...
            // TODO: currently, only propagated onto the Gaussian with the largest pixel response
            // // calculate cumulative visibility
            // if (accum_visible != nullptr && inside) {
            //     atomicAdd(accum_visible + g, vis);
            // }
    ...
    if (inside) {
        ...
        if (max_resp_vis > 0) {
            if (accum_times != nullptr) {
                atomicAdd(accum_times + max_resp_idx, 1);
            }
            if (accum_visible != nullptr) {
                atomicAdd(accum_visible + max_resp_idx, max_resp_vis);
            }
            if (query_values != nullptr) {
                GSPLAT_PRAGMA_UNROLL
                for (uint32_t k = 0; k < query_dim; ++k) {
                    gpuAtomicAdd(query_answers + max_resp_idx * query_dim + k,  max_resp_vis * query_values[pix_id * query_dim + k]);
                }
            }
        }
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, at::optional<torch::Tensor>, at::optional<torch::Tensor>, at::optional<torch::Tensor>, torch::Tensor> call_kernel_with_dim(
    // Gaussian parameters
    ...
    //return accum visible
    const bool ret_visible,
    // queries
    const at::optional<torch::Tensor> &query_values, // [C, image_height, image_width, K]
    const uint32_t query_dim,
    // image size
    const uint32_t image_width,
    const uint32_t image_height,
    ...
    if (query_values.has_value()) {
        GSPLAT_CHECK_INPUT(query_values.value());
    }
    ...
    at::optional<torch::Tensor> accum_times;
    if (ret_visible) {
        accum_times = torch::zeros(
            {C, N},
            means2d.options().dtype(torch::kInt32)
        );
    }

    at::optional<torch::Tensor> accum_visible;
    if (ret_visible) {
        accum_visible = torch::zeros(
            {C, N},
            means2d.options().dtype(torch::kFloat32)
        );
    }

    at::optional<torch::Tensor> query_answers;
    if (query_values.has_value()) {
        query_answers = torch::zeros(
            {C, N, query_dim},
            means2d.options().dtype(torch::kFloat32)
        );
    }
    ...
    rasterize_to_pixels_fwd_kernel<CDIM, float>
        <<<blocks, threads, shared_mem, stream>>>(
            ...
            query_values.has_value() ? query_values.value().data_ptr<float>() : nullptr,
            query_dim,
            image_width,
            image_height,
            tile_size,
            ...
            accum_times.has_value() ? accum_times.value().data_ptr<int32_t>() : nullptr,
            accum_visible.has_value() ? accum_visible.value().data_ptr<float>() : nullptr,
            query_answers.has_value() ? query_answers.value().data_ptr<float>() : nullptr,
            last_ids.data_ptr<int32_t>()
        );

    return std::make_tuple(renders, alphas, render_distort, accum_times, accum_visible, query_answers, last_ids);
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, at::optional<torch::Tensor>, at::optional<torch::Tensor>, at::optional<torch::Tensor>, torch::Tensor>
