...
__global__ void adam_kernel(
    ...
    const bool *valid,
    const scalar_t *__restrict__ step_per_param,
    const float lr,
    const float b1,
    const float b2,
    const float eps
    ...
    register_exp_avg_sq = b2 * register_exp_avg_sq + (1.0f - b2) *
                                                     ...
                                                     register_param_grad;
    // Bias correction
    float bias_correction1 = 1.0f - powf(b1, step_per_param[g_idx]);
    float bias_correction2 = 1.0f - powf(b2, step_per_param[g_idx]);
...
__global__ void increment_step_kernel(
    scalar_t *__restrict__ step_per_param,
    const bool* valid,
    int N
) {
...
void launch_adam_kernel(
    ...
    AT_DISPATCH_FLOATING_TYPES(param.scalar_type(), "increment_step_kernel", [&]() {
        increment_step_kernel<scalar_t>
            ...
            step_per_param.data_ptr<scalar_t>(),
