...
__global__ void fully_fused_projection_bwd_kernel(
    ...
    const T* __restrict__ gauss_confs, // unused
    // fwd outputs
...
)
{
    ...
    case CameraModelType::FISHEYE: // fisheye projection
        fisheye_proj_vjp<T>(
            ...
            v_covar_c);
        break;
    case CameraModelType::EQUIRECTANGULAR: // equirectangular projection
        equir_proj_vjp<T>(
            mean_c,
            covar_c,
            image_width,
    ...
    if (v_viewmats != nullptr) {
        if (gauss_confs!= nullptr) {
            gauss_confs += gid;
            v_R *= gauss_confs[0];
            v_t *= gauss_confs[0];
        } // use confidence to weight the gradient of viewmats
...
fully_fused_projection_bwd_tensor(
    ...
    const float eps2d,
    const CameraModelType camera_model,
    const at::optional<torch::Tensor>& gauss_confs, // unused
    // fwd outputs
    ...
    if (C && N) {
        fully_fused_projection_bwd_kernel<float>
            <<<(C * N + GSPLAT_N_THREADS - 1) / GSPLAT_N_THREADS,
                ...
                eps2d,
                camera_model,
                gauss_confs.has_value() ? gauss_confs.value().data_ptr<float>() : nullptr,
