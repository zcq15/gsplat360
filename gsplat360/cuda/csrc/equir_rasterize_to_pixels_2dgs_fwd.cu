...
/****************************************************************************
 * Equirectangular Rasterization to Pixels Forward Pass 2DGS
 ****************************************************************************/

template <uint32_t COLOR_DIM, typename S>
__global__ void equir_rasterize_to_pixels_fwd_2dgs_kernel(
    const uint32_t C,
    const uint32_t N,
...
)
{
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            const vec3<S> n_M = vec3<S>(n_M_ptr[0], n_M_ptr[1], n_M_ptr[2]);

            // n_M^T * (x - w_M) = n_M^T * (len * ray_dir - w_M) = 0
            // len * (n_M^T * ray_dir) = (n_M^T * w_M)
            // len = (n_M^T * w_M) / (n_M^T * ray_dir)

            S nM_wM = glm::dot(n_M, w_M);
            S nM_dir = glm::dot(n_M, ray_dir);
            if (abs(nM_dir) <= 1e-3f) {
                //                visible = false;
            ...
            const vec3<S> ray_cross = nM_wM / nM_dir * ray_dir;

            const S s_u = glm::length(u_M);
            const S s_v = glm::length(v_M);
            const S s_n = glm::length(n_M);

            // inverse(RS) = S^-1 * R^T = (S^-1 * S^-1) * (RS)^T
            const mat3<S> inv_S2 = mat3<S>(1.f / (s_u * s_u), 0.f, 0.f, 0.f, 1.f / (s_v * s_v), 0.f, 0.f, 0.f, 1.f / (s_n * s_n));
            const mat3<S> inv_RS = inv_S2 * glm::transpose(mat3<S>(u_M, v_M, n_M));
            const vec3<S> inv_p = -inv_RS * w_M;

            // s = (u, v, 0)^T
            const vec3<S> s = inv_RS * ray_cross + inv_p;
...
)
{
    ...
    const uint32_t shared_mem = tile_size * tile_size * (sizeof(int32_t) + sizeof(vec3<float>) + sizeof(vec3<float>) + sizeof(vec3<float>) + sizeof(vec3<float>));

    // TODO: an optimization can be done by passing the actual number of
    // channels into the kernel functions and avoid necessary global memory
    // writes. This requires moving the channel padding from python to C side.
    if (cudaFuncSetAttribute(
            equir_rasterize_to_pixels_fwd_2dgs_kernel<CDIM, float>,
...
std::tuple<
    ...
    torch::Tensor,
    torch::Tensor>
equir_rasterize_to_pixels_fwd_2dgs_tensor(
    // Gaussian parameters
    const torch::Tensor& means2d, // [C, N, 2] or [nnz, 2]
