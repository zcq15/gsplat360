...
/****************************************************************************
 * Equirectangular Projection of Gaussians (Batched) Backward Pass
 ****************************************************************************/
template <typename T>
__global__ void equir_fully_fused_projection_bwd_2dgs_kernel(
    // fwd inputs
    const uint32_t C,
    const uint32_t N,
...
)
{
    ...
    vec2<T> scale = glm::make_vec2(scales);
    ...
    mat3<T> RS_camera = R * quat_to_rotmat<T>(quat) * mat3<T>(scale[0], 0.0, 0.0, 0.0, scale[1], 0.0, 0.0, 0.0, 1.0);
    ...
    vec2<T> v_mean2d = glm::make_vec2(v_means2d);
    T v_depth = v_depths[0];
    vec4<T> v_quat(0.f);
    vec2<T> v_scale(0.f);
    vec3<T> v_mean(0.f);
    mat3<T> v_R(0.f);
    ...
    equir_compute_ray_transforms_aabb_vjp(
        mean,
        quat,
        scale,
        R,
...
equir_fully_fused_projection_bwd_2dgs_tensor(
    // fwd inputs
    ...
    const bool viewmats_requires_grad)
{
    ...
    if (C && N) {
        equir_fully_fused_projection_bwd_2dgs_kernel<float>
