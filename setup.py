...
__version__ = None
exec(open("gsplat360/version.py", "r").read())

URL = "https://github.com/zcq15/gsplat360"
...
def get_extensions():
    ...
    extensions_dir_v2 = osp.join("gsplat360", "cuda", "csrc")
    sources_v2 = glob.glob(osp.join(extensions_dir_v2, "*.cu")) + glob.glob(osp.join(extensions_dir_v2, "*.cpp"))
    ...
    if sys.platform == "win32":
        define_macros += [("gsplat360_EXPORTS", None)]
    ...
    glm_path = os.path.join(current_dir, "gsplat360", "cuda", "csrc", "third_party", "glm")
...
setup(
    ...
    url=URL,
    download_url=f"{URL}/archive/gsplat360-{__version__}.tar.gz",
    python_requires=">=3.7",
    install_requires=[
        "ninja",
        "numpy",
    ...
    extras_require={
        # dev dependencies. Install them by `pip install gsplat360[dev]`
        "dev": [
